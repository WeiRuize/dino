...
class Libero_DINOWM_Env(gym.Env):
    """Latent-space LIBERO env backed by a DINO-WM VideoTransformer.

    Mirrors the Franka DINO-WM env: reset() seeds a `num_frames` history window
    from an offline trajectory; step() rolls the world model forward in latent
    space with the proposed action and returns the immediate safety margin
    (negative == unsafe). The two camera streams map to:
        front (cam_zed)  <- agentview,  wrist (cam_rs) <- eye_in_hand.
    The observation is the per-patch mean of the transformer features
    (2*dim + ac_dim + state_dim = 786 for the default config).
    """
    ...
    def step(self, action):
        ...
        rew = self.safety_margin(latent)  # negative if unsafe
    ...
    def reset(self, initial_state=None, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        try:
            ...
            self._reset_loader()
            data = next(self.data)
        inputs2 = data['cam_rs_embd'][[0], :].to(self.device)    # wrist / eye_in_hand
        inputs1 = data['cam_zed_embd'][[0], :].to(self.device)   # front / agentview
