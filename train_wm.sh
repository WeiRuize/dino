...
# ---- 训练提速开关（按需覆盖）----------------------------------------------
# 把整份数据预载进内存，消除每步从 h5 随机读盘（GPU-Util 趋近 0 的根因）。
# 需放得下内存：先看 `ls -lh $LIBERO_WM_DATA` 和 `free -g`，放得下设 1，放不下保持 0。
export LIBERO_WM_IN_MEMORY="${LIBERO_WM_IN_MEMORY:-0}"
export LIBERO_WM_BS="${LIBERO_WM_BS:-16}"            # 单步 batch（调大填显存）
export LIBERO_WM_NUM_WORKERS="${LIBERO_WM_NUM_WORKERS:-8}"  # DataLoader worker 数
# ---------------------------------------------------------------------------
