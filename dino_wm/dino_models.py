...
# Per-dimension action bounds used by normalize_acs / unnormalize_acs.
# Defaults are the original Franka teleop ranges so existing Franka scripts keep
# their exact behaviour. LIBERO pipelines override these at runtime with stats
# computed from the dataset, via set_action_bounds() or load_action_bounds().
_DEFAULT_AC_MAX = [0.89928758, 0.71893158, 0.69869383, 0.32456627, 0.51343921, 0.28401476, 1.0]
_DEFAULT_AC_MIN = [-0.78933347, -1.0, -0.95038878, -0.3243517, -0.30636792, -0.30071826, -1.0]
_AC_MAX = list(_DEFAULT_AC_MAX)
_AC_MIN = list(_DEFAULT_AC_MIN)


def set_action_bounds(min_ac, max_ac):
    """Override the per-dim action normalization bounds (e.g. with LIBERO stats)."""
    global _AC_MIN, _AC_MAX
    _AC_MIN = [float(x) for x in np.asarray(min_ac, dtype=np.float64).reshape(-1)]
    _AC_MAX = [float(x) for x in np.asarray(max_ac, dtype=np.float64).reshape(-1)]


def load_action_bounds(path):
    """Load action bounds from an .npz with arrays 'min' and 'max'.

    The file is produced by dino_wm/libero_to_dataset.py over the LIBERO data.
    """
    data = np.load(path)
    set_action_bounds(data["min"], data["max"])


def normalize_acs(acs, device='cuda:0'):
    max_ac = torch.tensor(_AC_MAX, dtype=acs.dtype).to(device)
    min_ac = torch.tensor(_AC_MIN, dtype=acs.dtype).to(device)
